...
let main = (arr, queries) => {
    ...
    let buildTree = (l, r, treeI) => {
        ...
        tree[treeI] = lTree + rTree;
        return tree[treeI];
    }

    let executeSumQuery = (ts, te, qs, qe, ti) => {
        ...
        return executeSumQuery(ts, ts+mid, qs, qe, 2*ti+1) + executeSumQuery(ts+mid+1, te, qs, qe, 2*ti+2);
    }
    ...
    queries.forEach(q=>{
        ...
        console.log(`From ${q[0]} To ${q[1]} => Sum ==> ${executeSumQuery(0, arr.length-1, q[0], q[1], 0)}`);
    });
}
